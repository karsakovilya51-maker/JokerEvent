...
async def cmd_id(message: types.Message):
    """Команда для быстрого получения ID группы прямо от бота"""
...
async def process_phone_step(message: types.Message, state: FSMContext):
    ...
    # 1. Отправляем заявку СТРОГО В ГРУППУ (CHANNEL_ID из настроек)
    group_chat_id = config.CHANNEL_ID
    if group_chat_id:
        try:
            await bot.send_message(chat_id=group_chat_id, text=lead_text, parse_mode=ParseMode.MARKDOWN)
        except Exception as e:
            logging.error(f"Ошибка отправки лида в рабочий чат ({group_chat_id}): {e}")
    else:
        logging.warning("CHANNEL_ID не настроен в переменной окружения!")

    # 2. Отправляем пользователю в ЛС подтверждение бронирования
    user_text = (
...
# --- Веб-сервер для поддержки активности (Render / UptimeRobot) ---
